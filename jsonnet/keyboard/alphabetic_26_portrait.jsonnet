...
local swipe = import '../lib/swipeData-en.libsonnet';
local toolbar = import '../lib/toolbar-en.libsonnet';

local swipe_up = swipe.swipe_up;
local swipe_down = swipe.swipe_down;
local swipe_right = swipe.swipe_right;
local swipe_left = swipe.swipe_left;


local swipeStyles = import '../lib/swipeStyle.libsonnet';
local holdSymbolStyles = import '../lib/holdSymbolStyles.libsonnet';
// local theme = std.extVar('theme');  // 接收外部参数theme: 浅色/深色
// local orientation = std.extVar('orientation');  // 接收外部参数orientation: 方向

// local keyboardLayout = keyboardLayout_.getKeyboardLayout(theme);
...
local createButton(key, size, bounds, root, isUpper=true) = {
  ...
  [if std.objectHas(swipe_right, key) then 'swipeRightAction']: swipe_right[key].action,
  [if std.objectHas(swipe_left, key) then 'swipeLeftAction']: swipe_left[key].action,
...
swipeStyles.getStyle('en', theme) +
holdSymbolStyles.getStyle('pinyin', theme) +
